...
func registerLifecycleTests(cfg StorageConfig) {
	Describe(
		"SBR Functional - StorageBasedRemediationConfig Lifecycle ("+cfg.Name+")",
		Ordered,
		Label(
			labels.OperatorSBR,
			...
			nameA := sbrparams.SBRCLifecycleTestNameA + cfg.NameSuffix
			nameB := sbrparams.SBRCLifecycleTestNameB + cfg.NameSuffix

			BeforeAll(func() {
				By("Discovering storage class (required for agent DaemonSet creation)")

				rwxStorageClass = discoverStorageClass(cfg)

				By("Cleaning up any leftover lifecycle test SBRCs from prior runs")

				for _, staleName := range []string{nameA, nameB} {
				...
				staleDS := map[string]bool{
					sbrparams.SBRAgentDaemonSetPrefix + nameA: true,
					sbrparams.SBRAgentDaemonSetPrefix + nameB: true,
				}
			...
			It("Verify StorageBasedRemediationConfig CR create, patch, multi-instance, and delete lifecycle",
				reportxml.ID("88734"),
				func() {
					sbrcA := buildSBRCWithMode(nameA, rwxStorageClass, cfg, map[string]interface{}{
						"sbrTimeoutSeconds": int64(sbrparams.SBRCTimeoutSecondsMin),
					})
					sbrcB := buildSBRCWithMode(nameB, rwxStorageClass, cfg, map[string]interface{}{
						"sbrTimeoutSeconds": int64(sbrparams.SBRCTimeoutSecondsMin),
					})

					DeferCleanup(func() {
						for name, obj := range map[string]runtimeclient.Object{
							nameA: sbrcA.DeepCopy(),
							nameB: sbrcB.DeepCopy(),
						} {
					...
					Expect(APIClient.Create(context.TODO(), sbrcA)).To(Succeed(),
						"StorageBasedRemediationConfig %q must be created successfully", nameA)

					waitForSBRCReady(nameA)
					...
					dsNameA := sbrparams.SBRAgentDaemonSetPrefix + nameA
					...
					Expect(patchErr).ToNot(HaveOccurred(),
						"Failed to patch StorageBasedRemediationConfig %q", nameA)
					...
					Expect(APIClient.Delete(context.TODO(), sbrcA.DeepCopy())).To(Succeed(),
						"StorageBasedRemediationConfig %q must be deleted successfully", nameA)

					waitForDaemonSetGC(dsNameA)

					By("Step 4: Creating StorageBasedRemediationConfig B and verifying its DaemonSet becomes ready")

					dsNameB := sbrparams.SBRAgentDaemonSetPrefix + nameB

					Expect(APIClient.Create(context.TODO(), sbrcB)).To(Succeed(),
						"StorageBasedRemediationConfig %q must be created successfully", nameB)

					waitForSBRCReady(nameB)
					...
					Expect(APIClient.Delete(context.TODO(), sbrcB.DeepCopy())).To(Succeed(),
						"StorageBasedRemediationConfig %q must be deleted successfully", nameB)
