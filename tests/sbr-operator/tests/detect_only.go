...
func registerDetectOnlyTests(cfg StorageConfig) {
	Describe(
		"SBR Functional — detectOnlyMode ("+cfg.Name+")",
		Ordered,
		ContinueOnFailure,
		Label(labels.OperatorSBR), func() {
			var (
				...
				keepalivePods   []string
			)

			sbrcName := sbrparams.SBRCDetectOnlyTestName + cfg.NameSuffix
			nhcName := sbrparams.NHCDetectOnlyTestName + cfg.NameSuffix
			...
			BeforeAll(func() {
				...
				if rwxStorageClass == "" {
					Skip("No storage class found; set env var or deploy storage before running")
				}
				...
				Eventually(func() error {
					stale := buildSBRC(sbrcName, map[string]interface{}{})
					...
					"Pre-test cleanup of stale SBRC %s failed", sbrcName)

				By("Waiting for stale StorageBasedRemediationConfig to be fully removed before recreating")

				Eventually(func() error {
					staleCheck := buildSBRC(sbrcName, map[string]interface{}{})
				...
				detectOnlySBRC = buildSBRCWithMode(sbrcName, rwxStorageClass, cfg, map[string]interface{}{
				...
				Eventually(func() error {
					...
					func() string {
						return fmt.Sprintf(
							...
							sbrcName,
							sbrcReadinessDiagnostics(sbrcName, dsName))
					})
			})
				...
				if detectOnlySBRC != nil {
					if deleteErr := APIClient.Delete(context.TODO(), detectOnlySBRC); deleteErr != nil {
						...
						Eventually(func() error {
							...
							return fmt.Errorf("SBRC %s still present after delete", sbrcName)
						}, medik8sparams.DefaultTimeout, sbrparams.DefaultPollInterval).Should(Succeed())
					}
				}
			})
			...
			It("Verify SBRC detectOnlyMode field: spec reflection, storage-fault suppression, toggle, and DaemonSet GC",
				...
				Label(
					...
					Expect(getErr).ToNot(HaveOccurred(),
						"StorageBasedRemediationConfig %q must exist on the cluster", sbrcName)
					...
					Expect(deleteErr).ToNot(HaveOccurred(),
						"StorageBasedRemediationConfig %q must be deletable", sbrcName)
