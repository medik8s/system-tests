...
func registerNHCIntegrationTests(cfg StorageConfig) {
	Describe(
		"SBR Functional — NHC Integration ("+cfg.Name+")",
		Ordered,
		ContinueOnFailure,
		Label(labels.OperatorSBR), func() {
			...
			sbrcName := sbrparams.SBRCNHCTestName + cfg.NameSuffix
			nhcName := sbrparams.NHCTestName + cfg.NameSuffix
			...
			BeforeAll(func() {
				...
				if deleteErr := APIClient.Delete(context.TODO(), staleRef); deleteErr != nil &&
					...
					GinkgoT().Logf("Warning: pre-cleanup delete %s: %v", sbrcName, deleteErr)
				}

				Eventually(func() error {
					getErr := APIClient.Get(context.TODO(),
						types.NamespacedName{Name: sbrcName, Namespace: medik8sparams.OperatorNs},
						buildSBRC(sbrcName, map[string]interface{}{}))
					...
					return fmt.Errorf("SBRC %s still terminating", sbrcName)
				...
				default:
					expectedSpec := buildNHC(nhcName).Object["spec"]
					...
					if fmt.Sprintf("%v", existingSpec) != fmt.Sprintf("%v", expectedSpec) {
						Skip(fmt.Sprintf(
							"NodeHealthCheck %q exists with a different spec than the test requires — "+
								"delete it and re-run to let the test create its own.\n"+
								"  existing: %v\n  expected: %v",
							nhcName, existingSpec, expectedSpec))
					}
				...
				injectorPodName = sbrparams.NHCInjectorPodName + cfg.NameSuffix + "-" +
			...
			AfterAll(func() {
				...
				if nhcCreatedByUs && nhcCR != nil {
					...
					Eventually(func() error {
						...
						getErr := APIClient.Get(context.TODO(),
							types.NamespacedName{Name: nhcName}, obj)
						if k8serrors.IsNotFound(getErr) {
							return nil
						}
						...
						return fmt.Errorf("NodeHealthCheck %s still terminating", nhcName)
					}, sbrparams.SBRCReadyTimeout, sbrparams.DefaultPollInterval).Should(Succeed(),
						"NodeHealthCheck %s must be fully removed after AfterAll delete", nhcName)
				}
