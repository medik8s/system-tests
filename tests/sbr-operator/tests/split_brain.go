...
func registerSplitBrainTests(cfg StorageConfig) {
	Describe(
		"SBR Functional — Node Failure: Split-Brain (Storage Arbitration) ("+cfg.Name+")",
		Ordered,
		ContinueOnFailure,
		Label(labels.OperatorSBR), func() {
			...
			sbrcName := sbrparams.SBRCSplitBrainTestName + cfg.NameSuffix
			nhcName := sbrparams.NHCSplitBrainTestName + cfg.NameSuffix
			...
			BeforeAll(func() {
				...
				Expect(storageClass).ToNot(BeEmpty(),
					"Could not discover a storage class for mode %q", cfg.Name)
			...
			AfterAll(func() {
				...
				if nhcCreatedByUs && nhcCR != nil {
					if deleteErr := APIClient.Delete(context.TODO(), nhcCR); deleteErr != nil &&
						...
						Eventually(func() error {
							getErr := APIClient.Get(context.TODO(),
								types.NamespacedName{Name: nhcName},
								nhcCR.DeepCopy())
							...
							return fmt.Errorf("NHC %s still present", nhcName)
						}, medik8sparams.DefaultTimeout, sbrparams.DefaultPollInterval).Should(Succeed())
					}
				}
