...
func registerWatchdogPathTests(cfg StorageConfig) {
	Describe(
		"SBR Functional — Node Failure: Total Storage I/O Loss (Watchdog Path) ("+cfg.Name+")",
		Ordered,
		ContinueOnFailure,
		Label(
			labels.OperatorSBR,
			...
			sbrcName := sbrparams.SBRCWatchdogPathTestName + cfg.NameSuffix
			nhcName := sbrparams.NHCSBRTestName + cfg.NameSuffix
			...
			BeforeAll(func() {
				...
				rwxStorageClass = discoverStorageClass(cfg)

				if rwxStorageClass == "" {
					Skip("No storage class found; skipping total storage I/O loss test")
				}

				// This test blocks Ceph ports (3300, 6789, 6800-7300) to trigger SBRStorageUnhealthy.
				// Verify the StorageClass uses a Ceph provisioner (covers both CephFS and RBD).
				...
				if !strings.Contains(strings.ToLower(storageClass.Provisioner), "ceph") {
					Skip(fmt.Sprintf("StorageClass %q provisioner %q is not Ceph-backed; "+
						"iptables rules block Ceph ports only — skipping", rwxStorageClass, storageClass.Provisioner))
				}
			...
			It("Total storage I/O loss: watchdog fires, node reboots, fencing completes",
				...
				Label(
					...
					injectorPodName := sbrparams.InjectorPodName + cfg.NameSuffix + "-88880"
					...
					By("Injecting Ceph REJECT rules (INPUT + OUTPUT) on target node via nsenter")
					...
					Expect(execErr).ToNot(HaveOccurred(),
						"Failed to inject Ceph REJECT rules on node %s", targetNodeName)

					GinkgoWriter.Printf("Ceph REJECT rules injected on node %s\n", targetNodeName)

					By(fmt.Sprintf("Waiting for SBRStorageUnhealthy=True on target node (~18s with Ceph)"))
					...
					Eventually(func() error {
						...
						"SBRStorageUnhealthy must become True on node %s after Ceph REJECT injection", targetNodeName)
