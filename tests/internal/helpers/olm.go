...
func LogOLMDiagnostics(
	...
	namespace, subscriptionName, catalogName string,
	logf func(string, ...interface{}),
) {
	...
	sub, subErr := olm.PullSubscription(apiClient, subscriptionName, namespace)
	if subErr != nil {
		logf("  Subscription %s: not found (%v)\n", subscriptionName, subErr)
	} else {
		state := ""
		...
		logf("  Subscription %s: state=%s currentCSV=%s installedCSV=%s\n",
			subscriptionName, state, sub.Object.Status.CurrentCSV, sub.Object.Status.InstalledCSV)
